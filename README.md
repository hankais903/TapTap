...
| 參數 | 說明 | 預設 |
|---|---|---|
| `--bpm 128` | 強制指定 BPM（相位仍會自動擬合） | 自動 |
| `--first-beat 0.42` | 強制指定第一拍時間（秒） | 自動 |
...
每台裝置的喇叭 / 耳機輸出延遲和觸控延遲都不一樣（有線 10–40ms、藍牙 100–250ms、手機觸控 20–50ms）。
設定頁 → **計時校正** → **開始校正**：跟著節拍器按 24 下（約 15 秒），遊戲會量出你的「輸入偏移」並存起來。
戴藍牙耳機請在戴著的狀態下校正。
...
| 時間軸 | 公式 | 補償什麼 |
|---|---|---|
| 聽到的時間 `heardTime` | `ctx.currentTime − 開始時刻 − 輸出延遲` | 瀏覽器回報的 `outputLatency + baseLatency`（拿不到就用平台預設） |
| 判定用 | `heardTime(按下那一刻) − 輸入偏移` | 觸控 / 鍵盤事件延遲 + 你個人的習慣，由校正精靈量出 |
| 畫面用 | `heardTime + 視覺偏移` | 顯示管線延遲，一般不用動 |

另外還有「音訊偏移」把整份譜面平移（藍牙耳機才需要），以及 `meta.json` 可選的 `offset` 欄位（毫秒）做每首歌的微調。

遊戲中判定線下方有一條 **hit-error bar**：藍 = 早、橘 = 晚，白色三角是最近 20 次的平均；非 Perfect 的判定會顯示 EARLY / LATE。
結算頁會算平均偏差，若你穩定偏一邊（|平均| > 15ms 且 σ < 40ms）會直接給「套用建議」按鈕。
...
mp3 → librosa
       ├── beat_track       → 粗估 BPM
       ├── onset_strength   → 高解析 (hop 128) onset 包絡
       │     └── fit_beat_grid → 在 BPM ±4 內用 0.01 BPM 解析度找最貼合的等距格子 + 相位，
       │                          離整數很近就吸附到整數 BPM (DAW 做的歌幾乎都是整數)
       ├── onset_detect     → 抓音符起始 (鼓點/旋律重音)
       └── spectral_centroid → 各音點的頻率重心
```

> 為什麼不直接用 `beat_track` 的 tempo：它的值被量化在 `512/22050` 秒的整數倍週期上（例如 112.347、117.454、107.666），
> 跟真實 BPM 差 0.2–2.5%。拿它做等距格子，每拍差 1–11ms，幾十秒後就漂掉半拍 —— 這正是舊版 11 首歌「總是對不到拍」的原因。
